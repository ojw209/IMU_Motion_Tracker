...
void imudata()
{
  ...
  {                                // get the latest data if ready yet
    ...
    roll=fusion.getFusionPose().x()* RTMATH_RAD_TO_DEGREE*-1; // left roll is negative
    pitch=fusion.getFusionPose().y()* RTMATH_RAD_TO_DEGREE; // nose down is negative
    yaw=fusion.getFusionPose().z()* RTMATH_RAD_TO_DEGREE; // 0 Yaw = 270 magnetic, this gives left or right up to 180 degrees
    // float hdm = yaw-90;        // converts yaw to heading magnetic  
    // if (yaw < 90 && yaw >= -179.99) {  
    // hdm = yaw+270;  
    // } 
    ror=imu->getGyro().x()* RTMATH_RAD_TO_DEGREE*-1; // rate of roll
    rop=imu->getGyro().y()* RTMATH_RAD_TO_DEGREE; // rate of pitch
    roy=imu->getGyro().z()* RTMATH_RAD_TO_DEGREE; // rate of yaw
    
    ax=imu->getAccel().x()* RTMATH_RAD_TO_DEGREE*-1; // acceleration on the x-axis 
    ay=imu->getAccel().y()* RTMATH_RAD_TO_DEGREE; // acceleration on the y-axis 
    az=imu->getAccel().z()* RTMATH_RAD_TO_DEGREE; // acceleration on the z-axis
